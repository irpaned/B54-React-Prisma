...
export function RightBar() {
  ...
  // const { onFollow, isPending } = useFollow();
  const { suggests } = useSuggest();
  console.log("ini suggest", suggests);
    ...
    <Box bg="black" w="400px" h="100%" color="white" p="20px 15px 0 20px" m={0}>
      ...
      >
        ...
        <CardBody paddingTop="0">
          ...
          <Box marginTop="-5">
            <Heading fontSize="25px">{user.fullName}</Heading>
            <Text fontSize="md" color="grey">
              @{user.userName}
            </Text>
            <Text>{user.bio}</Text>
            <Box display="flex">
              <Text marginRight="4px">{user.TotalFolloweds}</Text>
              <Text color="grey">Followers</Text>
              <Text marginLeft="10px">{user.TotalFollowers}</Text>
